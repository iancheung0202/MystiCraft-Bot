...
def emoji_url(emoji_string: str) -> str | None:
    match = re.compile(r"<a?:([a-zA-Z0-9_]+):(\d+)>").match(emoji_string.strip())
    if not match:
        return None
...
async def load_emoji_icon(emoji_string: str) -> Image.Image | None:
    url = emoji_url(emoji_string)
    if url is None:
        return None
...
def vignette_mask(w: int, h: int) -> Image.Image:
    left_mask = Image.new("L", (w, h), 0)
    lm_draw = ImageDraw.Draw(left_mask)
    left_strip_width = int(w * 0.60)
    left_max_alpha = 235
    ...
    corner_max_alpha = 225 
    cx, cy = w * 0.95, h * 0.98
    max_radius = w * 0.58 
    step = 2
    for y0 in range(int(h * 0.30), h, step): 
        for x0 in range(int(w * 0.38), w, step): 
...
def draw_shadowed_text(draw: ImageDraw.ImageDraw, pos, text, font, fill=COLOR_WHITE, shadow_offset=3):
    x, y = pos
    draw.text((x + shadow_offset, y + shadow_offset), text, font=font, fill=COLOR_SHADOW)
    draw.text((x, y), text, font=font, fill=fill)


def paste_icon(base: Image.Image, icon: Image.Image | None, pos, size: int):
    if icon is None:
        return
...
async def generate_status_banner(stats: dict) -> io.BytesIO:
    ...
    banner_bytes_task = download(SERVER_BANNER_URL)
    indicator_icon_task = load_emoji_icon(indicator_emoji)
    players_icon_task = load_emoji_icon(EMOJI_PLAYERS)
    connect_icon_task = load_emoji_icon(EMOJI_CONNECT)
    peak_today_icon_task = load_emoji_icon(EMOJI_PEAK_TODAY)
    peak_alltime_icon_task = load_emoji_icon(EMOJI_PEAK_ALLTIME)
    avg_icon_task = load_emoji_icon(EMOJI_AVG)
    ...
    mask = vignette_mask(BANNER_W, BANNER_H)
    ...
    # Server name
    draw_shadowed_text(draw, (PAD_LEFT, y), "MYSTICRAFT", font_bold(66))
    y += 100
    ...
    if indicator_icon is not None:
        ...
        icon_cy = y + 26
        paste_icon(base, indicator_icon,
    ...
    draw_shadowed_text(draw, (label_x, y),
                        status_label, font_bold(50), fill=indicator_color)
    y += 74
    draw_shadowed_text(draw, (PAD_LEFT, y), status_sub, font_reg(27), fill=COLOR_GREY)
    y += 82
    ...
    paste_icon(base, players_icon, (PAD_LEFT, y + 4), size=icon_size_players)
    draw_shadowed_text(draw, (PAD_LEFT + icon_size_players + 14, y),
    ...
    paste_icon(base, connect_icon, (PAD_LEFT, icon_y), size=icon_size_connect)
    draw_shadowed_text(draw, (PAD_LEFT + icon_size_connect + 12, hdr_y),
                        "HOW TO CONNECT", font_bold(30))
    draw_shadowed_text(draw, (PAD_LEFT, platform_y), platform_line, font_reg(22), fill=COLOR_GREY)
    draw_shadowed_text(draw, (PAD_LEFT, ip_y),   ip_line,   font_reg(28))
    draw_shadowed_text(draw, (PAD_LEFT, port_y), port_line, font_reg(28))
    ...
    for icon, label, value in rows:
        ...
        vw = vbbox[2] - vbbox[0]
        draw_shadowed_text(draw, (text_x - vw, yy), value, vf)
        paste_icon(base, icon, (icon_x, yy - 4), size=stat_icon_size)

        lbbox = draw.textbbox((0, 0), label, font=lf)
        lw = lbbox[2] - lbbox[0]
        draw_shadowed_text(draw, (text_x - lw, yy + 38), label, lf, fill=COLOR_GREY)
        yy += row_h
...
class StatusContainer(discord.ui.Container):
    def __init__(self, banner_file: discord.File, timestamp: int):
        super().__init__()
        self.banner = discord.ui.MediaGallery(discord.MediaGalleryItem(banner_file))
        self.add_item(self.banner)
        self.footer = discord.ui.TextDisplay(f"-# {EMOJI_CLOCK} Last updated <t:{timestamp}:R>")
